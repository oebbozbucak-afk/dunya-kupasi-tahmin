...
// Bağlantıyı daha kararlı hale getiriyoruz
...
const Tahmin = mongoose.model('Tahmin', new mongoose.Schema({ 
    isim: { type: String, unique: true }, 
    tahminler: Object, 
    puan: { type: Number, default: 0 } 
}));

const Sonuc = mongoose.model('Sonuc', new mongoose.Schema({ 
...
// Tahmin kaydetme (Daha dayanıklı hale getirildi)
app.post('/api/tahmin-kaydet', async (req, res) => {
    try {
        const { isim, tahminler } = req.body;
        if (!isim) return res.status(400).send("İsim gerekli");
