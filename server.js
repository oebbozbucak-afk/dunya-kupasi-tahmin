...
app.post('/api/admin/tahmin-sil', async (req, res) => {
    if (req.body.sifre !== "ordu52") return res.status(401).send();
...
// YENİ: İsimle Maç Silme
app.post('/api/admin/mac-sil-isimle', async (req, res) => {
    if (req.body.sifre !== "ordu52") return res.status(401).send();
    await Mac.findOneAndDelete({ isim: req.body.isim });
